...
> 매 시간 슬롯에 짧은순간의 영상을 기록하고, 구성원들의 하루일상을 공유하는 영상 기반 소셜 앱입니다.
...
- 00:00 ~ 24:00 하루 간 영상 하나의 비디오파일로 추출 시스템 (추후 업데이트)
...
- Video: Cached Video Player
- Architecture: Screen / Service / Model / Widget

---

## 🛠️ App Store Review Rejection & Resolution - ios 앱 제출 및 심사과정
...
- **유저 차단(Block) 기능:** 불량 사용자의 피드와 댓글을 차단할 수 있는 버튼 구현.
...
## :star: Technical Challenges - 문제해결 과정
...
#### 문제: 회원가입 과정에서 Apple 로그인, 카카오 로그인, Passkey 등 다양한 인증 방식을 함께 제공하는 방안을 검토했습니다. 
...
#### 문제: 기존에는 시간대마다 컨트롤러를 생성해 필수적으로 영상로딩 렉이 동반되었습니다. 
...
✅ 업데이트 ( 1.0.2버전 ): 그럼에도 여전히 앱을 빌드하고 접속했을때 영상과 프로필사진을 가져오는데있어 약간의 잔렉이 발생함을 확인할 수 있었습니다. 이 부분을 개선해 최대한 사용성에 불편감을 없애는데 집중할 예정입니다.
...
## 향후 업데이트 계획
* 영상 추출 기능추가 - 하루 간 00:00 ~ 24:00 의 영상목록을 하루로그형식으로 추출해주는 기능 추가예정
* 카카오 로그인 구현
* 기타 소셜 로그인 , 애플 로그인 구현
...
* 이용 등급 연령대 연령 낮추기
* 앱 용량 사이즈 다운 - 불필요한 테스트 파일 제거
* 카메라 버튼 및 텍스트 부자연스러운 ui 개선
* 
### 1.0.2 버전 업데이트 계획 - 진행중
* 영상 추출 기능추가 - 하루 간 00:00 ~ 24:00 의 영상목록을 하루로그형식으로 추출해주는 기능 추가예정
